...
struct PartyView: View {
    ...
    let partyStore: StoreOf<PartyCore>
    
    init(partyStore: StoreOf<PartyCore>) {
        self.partyStore = partyStore
    }
    
    var body: some View {
        WithViewStore(self.partyStore, observe: { $0 }) { viewStore in
            ZStack {
                GeometryReader { geo in
                    VStack {
                        if let partyName = viewStore.partyName, let partyLists = viewStore.partyLists {
                            Text("\(partyName) (\(partyLists.count)명)")
                        ...
                        PartyListSectionView(
                            members: viewStore.partyLists ?? [],
