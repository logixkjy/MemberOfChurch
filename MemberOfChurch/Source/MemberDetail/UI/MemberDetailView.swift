...
struct MemberDetailView: View {
    ...
    let memberDetailStore: StoreOf<MemberDetailCore>
    
    init(memberDetailStore: StoreOf<MemberDetailCore>) {
        self.memberDetailStore = memberDetailStore
    }
    
    var body: some View {
        WithViewStore(self.memberDetailStore, observe: { $0 }) { viewStore in
            WithViewStore(mainRouter.memberRegisterStore, observe: { $0 }) { viewRegisterStore in
                ZStack {
                    GeometryReader { geo in
                        VStack {
                            ...
                            ScrollViewReader { proxy in
                                ScrollView {
                                    MemberDetailInfoView(
                                        memberDetail: viewStore.memberDetail,
                                        familyMemberList: viewStore.familyMemberList,
                                        onSelect: { psnId in
                                            if let id = psnId {
                                                viewStore.send(.setPsnId(id))
                                                viewStore.send(.getMemberDetailInfo)
                                            }
                        ...
                        .task {
                            viewStore.send(.getMemberDetailInfo)
                            viewRegisterStore.send(.onAppear)
                        }
                    }
